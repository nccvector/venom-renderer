...
public:
    ...
    glm::vec3 trace(Ray ray, int bounces = 1, int samples = 8) {
        ...
        for (int i = 0; i < samples; i++) {
            ...
            // Oren-nayar
            glm::vec3 brdf = hitInfo.face->mat->BRDF(diffuse_ray.direction, -ray.direction, hitInfo.normal,
                                                     hitInfo.texcoord);

            indirectLight += hitInfo.face->mat->baseColor * hitInfo.face->mat->emission;
            indirectLight += brdf * trace(diffuse_ray, bounces - 1, 1) *
                             glm::dot(hitInfo.normal, diffuse_ray.direction) * lightImportanceSamplingWeight /
                             uniformPDF;
        }
    ...
    void colorFromRegion(glm::vec4 region, glm::vec3 *colors) {
        ...
        for (int y = 0; y < (region.w - region.z); y++) {
            for (int x = 0; x < (region.y - region.x); x++) {
                ...
                colors[x + y * (int) (region.y - region.x)] = this->trace(
                        this->transRays[int(y + region.z)][int(x + region.x)]);
            }
        }
    }
};